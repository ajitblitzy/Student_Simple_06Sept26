 ...
 *   - a row that is not a plain object keyed by column letter - an array, an
 *     exotic object or a primitive - or a cell holding anything but a string;
 *     `lib/workbook.js` can produce neither, so such a row can only arrive
 *     through `fromRows`, and reading it as blank would skip a student in
 *     silence instead of reporting the malformed input.
 *
 * One condition is deliberately *not* fatal: a **blank key cell skips the row**,
 * which is what lets a maintainer leave trailing empty rows in a workbook. That
 * is an *empty* cell (`''`) or a column the worksheet omits - never a `null`,
 * which is not a value the reader can yield.
 *
 ...
 *
 * Coercing would be the more dangerous choice, which is the point: a `null` key
 * read as `''` makes the row look like the trailing blank row a maintainer left
 * behind, so it is skipped and the student silently disappears from the
 * directory. `fromRows` is the no-file equivalent of `load` and must be exactly
 * as strict as it (AAP 0.7.3), so a shape the reader could never produce is
 * fatal here too, with the row and cell named.
 *
 ...
 * blank name, malformed row shape, skipped blank row - without adding a binary
 * fixture.
 *
 * The validation applied here is **identical** to the validation `load` applies
 * (AAP 0.7.3): injected construction is no more permissive than construction
 * from a file, so a row shape the reader could never yield is rejected rather
 * than reinterpreted as absent data.
 *
