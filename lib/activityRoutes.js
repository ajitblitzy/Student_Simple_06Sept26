 ...
 * contract. It becomes `409`; every remaining rejection becomes the one `500`.
 */
 ...
 * The repository list is the module's published interface in full, so a
 * wiring mistake is a construction failure rather than a `TypeError` in the
 * middle of a request.
 */
 ...
 *
 * **A present parameter is a filter whatever it holds.** `URLSearchParams.get`
 * returns `null` for an absent parameter and a string for a present one,
 * however little that string holds: `''` for `?activity=` and for a bare
 * `?activity`, `' '` for `?activity=%20`. Absence and a blank value must not
 * collapse into one signal - an absent parameter means the whole roster, while
 * a blank one is a filter that matches nothing, because no group's activity key
 * can be empty. Returning `undefined` for a blank value would answer it with
 * all eight groups instead of `{count: 0, activities: []}`.
 *
 * The type test is what carries that distinction, so a `query` object that
 * answers with something other than a string or `null` is read as absence
 * rather than handed on as a filter of an unusable type.
 *
 * @param {{query?: URLSearchParams}} route The boundary object.
 * @returns {string|undefined} The parameter's value when it is present -
 *   including the empty string - or `undefined` when it is absent.
 */
 ...
 * every remaining failure becomes the one `500`, with the real error on stderr.
 * The chain ends in a catch of its own, so neither a rejection nor a failure
 * while writing the response can ever become an unhandled rejection.
 *
 * @param {{directory: object, repository: object}} deps The injected context.
 * @param {{responded: boolean}} state Per-request write state.
 * @param {import('http').IncomingMessage} req The request.
...
/**
 * Handles a `POST` in the fixed order: media type, then body size, then parsing
 * and field validation, then the write. The identifier has already been
 * validated and resolved by the caller, which is why an unknown student
 * outranks an oversize or unparseable body.
 *
 * The media type is examined **before a single body byte is read**, so a
 * request this service cannot interpret is refused without being buffered, and
 * its `415` is an early error - `Connection: close`, request stream abandoned -
 * exactly like the identifier failures the caller decided.
 *
 * @param {{directory: object, repository: object}} deps The injected context.
 * @param {{responded: boolean}} state Per-request write state.
 * @param {import('http').IncomingMessage} req The request.
