 ...
 *
 * Such a request needs deciding twice over: the response has to reach a client
 * that is still uploading, and the body already on the wire must not leave that
 * client waiting for a drain that never comes. Both mechanics below were
 * measured on the target runtime, 20 iterations per variant with a 1 MiB body
 * in flight, against a keep-alive agent and a fresh-connection agent:
 *
 *   | what the handler does after `res.end(payload)` | responses delivered |
 *   | ---------------------------------------------- | ------------------- |
 *   | `req.destroy()` immediately                    |   0/20 - ECONNRESET |
 *   | `req.destroy()` on the response's `'finish'`   |   0/20 - ECONNRESET |
 *   | `req.resume()` - drain, never abort            |  20/20, within 9 ms |
 *   | nothing at all                                 |  20/20, within 5 ms |
 *
 * So:
 *
 *   1. **Drain, never abort.** Destroying the request resets the connection,
 *      and a reset discards whatever the kernel has not yet put on the wire -
 *      including the response just written, every single time. `req.resume()`
 *      instead puts the request into flowing mode with no consumer attached, so
 *      the rest of the upload is read and discarded: the client's write
 *      completes, the drain genuinely happens, and the response survives.
 *      Draining is only started once the request is not already complete, and
 *      `'error'` is muted first so an upload the client abandons mid-drain does
 *      not surface as an unhandled `'error'` on the request.
 *   2. **`Connection: close`.** A connection whose request body this module
 *      chose not to interpret is not one to reuse, and declaring that is what
 *      keeps a keep-alive client from pipelining a second request behind a body
 *      that was discarded. It costs nothing: the measured latency above is the
 *      full round trip including the close.
 *
...
const logInternalError = (context, error) => {
  const detail = error instanceof Error
    ...
    : String(error);
  console.error(`activityRoutes: ${context}: ${detail}`);
};
 ...
 * where the startup banner's stream already is. This module never calls
 * `process.exit` - turning a failure into process behaviour belongs to the
...
const handleActivitiesRoute = (deps, state, req, res, route, method, rawPath) => {
  ...
  let activities;
  try {
    // Mapping stays inside the guard: a dependency that answered with something
    // other than the documented shape must fail as an internal fault with the
    // detail on stderr, never as an exception thrown back at the entrypoint.
    activities = deps.repository.listActivities(readActivityFilter(route)).map((group) => {
      const studentIds = group.studentIds.slice();
      return { activity: group.activity, count: studentIds.length, studentIds };
    });
  } catch (error) {
    sendInternalError(state, req, res, 'listing activities', error);
    return;
  }
...
const handleStudentActivitiesRead = (deps, state, req, res, studentId) => {
  ...
  let name;
  try {
    activities = deps.repository.listByStudent(studentId).map((record) => ({
    ...
    name = deps.directory.nameOf(studentId);
  } catch (error) {
    sendInternalError(state, req, res, `listing activities for ${studentId}`, error);
    return;
  }
...
const recordActivity = (deps, state, req, res, studentId, activity) => {
  Promise.resolve()
    ...
    .catch((error) => {
      ...
      sendInternalError(state, req, res, `recording "${activity}" for ${studentId}`, error);
    })
    .catch((error) => {
 ...
 *
 * The identifier echoed in either failure is the **raw** segment recovered from
 * `rawPath`, not the decoded value, so a caller sees back exactly what they
 * sent.
 *
 * @param {{directory: object, repository: object}} deps The injected context.
 * @param {{responded: boolean}} state Per-request write state.
