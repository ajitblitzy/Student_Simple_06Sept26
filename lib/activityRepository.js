 ...
 * so `lib/activityRoutes.js` selects `409`, `503`/`507` or `500` without matching
 * strings: `ACTIVITY_ALREADY_RECORDED`, `ACTIVITY_PERSIST_FAILED`, the three
 * bound refusals below, and `INVALID_ACTIVITY`, `INVALID_STUDENT_ID` or
 * `STUDENT_NOT_FOUND` for arguments that never reach the queue.
 *
 * The write path is bounded. Validation judges one request; four configurable
 * ceilings, all enforced here because this module owns both the queue and the
 * file (`WRITE_LIMIT_DEFAULTS` carries their defaults), bound what many requests
 * may do: `maxPendingWrites` (admitted, unsettled writes -
 * `ACTIVITY_WRITE_QUEUE_FULL`), `maxActivitiesPerStudent` (records one student
 * may hold across both sources - `STUDENT_ACTIVITY_LIMIT_REACHED`), and
 * `maxRegistryRecords`/`maxRegistryBytes` (the registry's cardinality and its
 * serialized size - `ACTIVITY_REGISTRY_FULL`). `reserveWrite(id)` acquires one
 * unit of capacity synchronously, so the request boundary can hold a slot from
 * before it reads a body until the write settles: N simultaneous requests take
 * N distinct slots and the next is refused, where N checks against an empty
 * queue would all have passed. `checkWriteAdmission(id)` is the same judgement
 * without the acquisition. A refusal writes nothing, enqueues nothing and leaves
 * both views untouched. These are finite capacity per process, not a per-client
 * rate limit.
 *
...
// The three refusals that bound the write path rather than judging its
// arguments. They are their own discriminators because a caller has to tell
// "retry shortly" apart from "this collection is full" apart from "the registry
// is full", and because none of them is a fault in the request: a queue-full
// refusal is temporal, and both quota refusals are a standing condition until
// records are removed.
const ERROR_WRITE_QUEUE_FULL = 'ACTIVITY_WRITE_QUEUE_FULL';
const ERROR_STUDENT_LIMIT = 'STUDENT_ACTIVITY_LIMIT_REACHED';
const ERROR_REGISTRY_FULL = 'ACTIVITY_REGISTRY_FULL';

/**
 * The bounds on pending and persisted work, and the values that apply when a
 * caller configures none. Without them every queued `POST` would hold a task on
 * the single-writer queue and every accepted one would add a record for good,
 * both growing with the caller rather than with the data (CWE-770, CWE-400).
 * The numbers are generous for this feature's data and still bound the cost of
 * abuse: writes are serialized one synchronous rewrite at a time, so a queue
 * deeper than 16 only adds latency; 32 activities is far beyond any real roster
 * of clubs; 1000 records is a hundred times the committed count; and 262144
 * bytes is the backstop, since 1000 records of a 64-character activity
 * serialize to roughly 100 KB, so the record count normally binds first and the
 * byte ceiling catches a hand-edited registry already larger than its count
 * suggests. Exported so `server.js` resolves its defaults against these exact
 * values instead of restating them.
 */
const WRITE_LIMIT_DEFAULTS = Object.freeze({
  maxPendingWrites: 16,
  maxActivitiesPerStudent: 32,
  maxRegistryRecords: 1000,
  maxRegistryBytes: 262144
});

const WRITE_LIMIT_KEYS = Object.freeze(Object.keys(WRITE_LIMIT_DEFAULTS));
...
/**
 * The refusal for a write that would exceed a bound.
 *
 * One builder for all three bounds, because the shape a caller branches on is
 * the same in every case: a stable `code`, the `limit` that was reached, and -
 * for the per-student bound - the identifier whose collection is full.
 * `lib/activityRoutes.js` branches on `code` alone to select the status and the
 * fixed sentence it sends. `limit` is **diagnostic detail, not part of any
 * client-facing message**: the routes deliberately do not disclose a configured
 * ceiling, so it exists here for a library caller and for this message, which is
 * for logs.
 *
 * Nothing about the request is echoed into the message beyond the normalized
 * identifier: a refusal that persisted nothing must not turn a caller's text
 * into a diagnostic (CWE-117), and the caller's activity name is not what these
 * refusals are about.
 *
 * @param {string} code One of the three bound discriminators.
 * @param {string} summary What was reached, with the limit named.
 * @param {number} limit The configured ceiling that refused the write.
 * @param {string} [studentId] The normalized identifier, for the per-student
 *   bound only.
 * @returns {Error} The error to reject with, never thrown synchronously.
 */
const createLimitError = (code, summary, limit, studentId) => {
  const error = new Error(`Activity repository: ${summary}`);
  error.code = code;
  error.limit = limit;
  if (studentId !== undefined) error.studentId = studentId;
  return error;
};

/**
 * Turns an admission refusal - the object `checkWriteAdmission` returns - into
 * the error `addActivity` rejects with, so the pre-enqueue gate and the
 * rejection can never disagree about which bound was reached.
 *
 * @param {{code: string, limit: number, studentId?: string, summary: string}} refusal
 *   The refusal describing the bound that was reached.
 * @returns {Error} The error to reject with.
 */
const errorForRefusal = (refusal) =>
  createLimitError(refusal.code, refusal.summary, refusal.limit, refusal.studentId);
...
/**
 * Resolves the four write bounds from a caller's options, falling back to
 * `WRITE_LIMIT_DEFAULTS` for each one omitted. Numbers only, and integers at
 * that: `server.js` hands over values it has already validated, so a string
 * arriving here means a caller bypassed that resolution, and coercing it would
 * let `'1.5'` or `'16abc'` become a silently different ceiling. One is the
 * floor, because a ceiling of `0` would refuse every write while reporting a
 * capacity condition rather than the configuration mistake it is.
 *
 * @param {object} options The caller's options object.
 * @returns {{maxPendingWrites: number, maxActivitiesPerStudent: number, maxRegistryRecords: number, maxRegistryBytes: number}}
 *   The frozen, fully concrete bounds.
 * @throws {Error} With `code === 'ACTIVITY_REPOSITORY_INVALID'`, naming the
 *   option and what was received.
 */
const resolveWriteLimits = (options) => {
  const limits = {};
  WRITE_LIMIT_KEYS.forEach((key) => {
    const candidate = options[key];
    if (candidate === undefined) {
      limits[key] = WRITE_LIMIT_DEFAULTS[key];
      return;
    }
    if (typeof candidate !== 'number' || !Number.isInteger(candidate) || candidate < 1) {
      throw createUsageError(
        `${key} must be an integer of at least 1 (received ${
          typeof candidate === 'number' ? String(candidate) : describeValue(candidate)
        })`
      );
    }
 ...
 *   optional labels used only in error messages. The four `max…` bounds are the
 *   write-path ceilings; each one omitted takes its value from
 *   `WRITE_LIMIT_DEFAULTS`, and each supplied value must be an integer of at
 *   least 1.
 * @returns {{
 ...
 *   recordCount: () => number,
 *   reserveWrite: (id: unknown) => {admitted: true, release: () => void}|{admitted: false, refusal: {code: string, limit: number, studentId?: string, summary: string}},
 *   checkWriteAdmission: (id: unknown) => {code: string, limit: number, studentId?: string, summary: string}|null
...
const fromData = (options) => {
  ...
  // Resolved before any row is read, so a mistyped ceiling fails while the
  // server is being built rather than on the first `POST`.
  const writeLimits = resolveWriteLimits(options);
  ...
  // The workbook records. Iterating `directory.ids()` rather than the worksheet
  // is what makes within-group ordering follow workbook row order without
  // re-deriving it, and it is why the directory exposes `ids()` at all.
  ...
  // own rejection. The queue is bounded: `pendingWrites` counts the writes that
  // have been admitted and have not settled, and a write that would exceed
  // `maxPendingWrites` is refused before it is enqueued, so neither the queue
  // nor the work waiting on it grows with the number of callers (CWE-770).
  let tail = Promise.resolve();

  let pendingWrites = 0;

  // `enqueue` does not count the task: pending work is counted by a reservation
  // acquired before this point, so the request boundary can hold a slot while
  // it reads a body - long before there is a task to enqueue.
  const enqueue = (task) => {
  ...
  /**
   * How many records a student already holds, across both sources. This is what
   * the per-student bound counts, because it is also what a per-student
   * response serializes.
   *
   * @param {string} studentId The normalized identifier.
   * @returns {number} The record count, `0` for a student holding none.
   */
  const countForStudent = (studentId) => {
    const records = byStudent.get(studentId);
    return records === undefined ? 0 : records.length;
  };

  /**
   * The two **quota** bounds, judged from the identifier alone: how many
   * records the registry holds, and how many this student holds. Separate from
   * the queue bound because they are re-judged in places where the queue bound
   * must not be - inside a critical section, and for a write that already holds
   * its slot - and re-testing a bound a write is itself counted against would
   * refuse the very write that bound admitted.
   *
   * @param {string} studentId The already-normalized identifier.
   * @returns {{code: string, limit: number, studentId?: string, summary: string}|null}
   *   The refusal, or `null` when both quotas have room. Never throws.
   */
  const quotaRefusalFor = (studentId) => {
    if (registryStore.length >= writeLimits.maxRegistryRecords) {
      return Object.freeze({
        code: ERROR_REGISTRY_FULL,
        limit: writeLimits.maxRegistryRecords,
        summary: `the registry holds ${registryStore.length} records, which is the configured `
          + `maximum of ${writeLimits.maxRegistryRecords}`
      });
    }

    const held = countForStudent(studentId);
    if (held >= writeLimits.maxActivitiesPerStudent) {
      return Object.freeze({
        code: ERROR_STUDENT_LIMIT,
        limit: writeLimits.maxActivitiesPerStudent,
        studentId,
        summary: `${KEY_HEADER_LABEL} ${studentId} already holds ${held} activities, which is `
          + `the configured maximum of ${writeLimits.maxActivitiesPerStudent}`
      });
    }
  ...
  /**
   * Whether one more write could be admitted for a student, and if not, which
   * bound would refuse it.
   *
   * **Inspection only** - it acquires nothing, so two callers asking at once
   * both get `null` and neither has reserved anything. Acquiring is
   * `reserveWrite`, and that is what the write path uses; this is here for a
   * caller that wants to read capacity without taking any: a test proving the
   * bound is engaged, or an operator reading the state of the queue.
   *
   * @param {unknown} id The student's identifier, normalized here so a caller
   *   that has not normalized still gets the right answer.
   * @returns {{code: string, limit: number, studentId?: string, summary: string}|null}
   *   `null` when a write could proceed, otherwise the refusal - `code` selects
   *   the response and `summary` is the operator-facing sentence. Never throws.
   */
  const checkWriteAdmission = (id) => {
    if (pendingWrites >= writeLimits.maxPendingWrites) {
      return Object.freeze({
        code: ERROR_WRITE_QUEUE_FULL,
        limit: writeLimits.maxPendingWrites,
        summary: `${pendingWrites} activity writes are already in flight, which is the `
          + `configured maximum of ${writeLimits.maxPendingWrites}`
      });
    }
    return quotaRefusalFor(directory.normalize(id));
  };

  /**
   * **Acquires** one unit of write capacity, or reports why it cannot.
   *
   * This is the primitive the bound is actually made of, and the difference
   * between it and `checkWriteAdmission` is the whole point: a check tells a
   * caller what the state is, and by the time the caller acts on it every other
   * caller has been told the same thing. A reservation is taken, so N
   * simultaneous requests acquire N distinct slots and the (N+1)th is refused.
   *
   * It is acquired at the request boundary **before a body is read**, which is
   * what makes the bound cover the expensive part: the socket, the listeners
   * and the buffered chunks of a request in progress, not merely the task it
   * eventually becomes. A slow or never-completed upload therefore occupies a
   * slot for exactly as long as it occupies the process.
   *
   * Releasing is the holder's obligation and is **idempotent**, so the boundary
   * may release on any exit path, ownership may pass to `addActivity`, and a
   * double release cannot corrupt the count.
   *
   * @param {unknown} id The student's identifier.
   * @returns {{admitted: true, release: () => void}|{admitted: false, refusal: {code: string, limit: number, studentId?: string, summary: string}}}
   *   A frozen handle carrying the release, or the refusal. Never throws.
   */
  const reserveWrite = (id) => {
    const refusal = checkWriteAdmission(id);
    if (refusal !== null) return Object.freeze({ admitted: false, refusal });
  ...
  /**
   * Whether a value is a live write reservation this module handed out. A value
   * that is anything else - absent, refused, or not a handle at all - means the
   * caller holds no slot, and `addActivity` then acquires one itself rather than
   * proceeding unbounded.
   *
   * @param {unknown} value The candidate reservation.
   * @returns {boolean} `true` for an admitted reservation handle.
   */
  const isWriteReservation = (value) =>
    isObject(value) && value.admitted === true && typeof value.release === 'function';
   ...
   *   2. re-check the record and per-student ceilings, for the same reason: the
   *      boundary judged them before a body was read and before every write
   *      already queued, so either can have been reached since;
   *   3. serialize the whole registry;
   *   4. measure the serialized bytes against the size ceiling - the one bound
   *      that cannot be judged any earlier, because it is a property of the
   *      bytes this write would produce;
   *   5. hand it to the writer, which writes the scratch file and renames it
   *      over the target;
   *   6. update both views **only after** the rename, so a record is never
   *      visible to a reader that was never persisted;
   *   7. resolve with the created record.
   *
   * Steps 1, 2 and 4 all refuse **before** anything is written, so every one of
   * them leaves the registry file and both views exactly as they were.
   *
   ...
   *   `STUDENT_ACTIVITY_LIMIT_REACHED` or `ACTIVITY_REGISTRY_FULL` for a
   *   ceiling reached, `ACTIVITY_PERSIST_FAILED` when the write did not land.
   */
  const persistActivity = async (studentId, activity) => {
    ...
    // Re-checked **inside** the section, exactly like the duplicate above: the
    // boundary judged these quotas before a body was read and before every
    // write already queued, so either can have reached its ceiling in between.
    // This is the check that cannot be raced. The queue bound is deliberately
    // not among them - this write holds a slot in it.
    const quotaRefusal = quotaRefusalFor(studentId);
    if (quotaRefusal !== null) {
      throw errorForRefusal(quotaRefusal);
    }
    ...
    // The hard size bound, measured on the **bytes that would be written**
    // rather than on a record count, because that is the only quantity that
    // bounds the file, the full-file rewrite cost and the memory the next load
    // will need. Nothing has been written at this point, so a refusal here
    // leaves the registry and both views exactly as they were.
    const contentBytes = Buffer.byteLength(contents, 'utf8');
    if (contentBytes > writeLimits.maxRegistryBytes) {
      throw createLimitError(
        ERROR_REGISTRY_FULL,
        `writing this activity would take the registry to ${contentBytes} bytes, past the `
          + `configured maximum of ${writeLimits.maxRegistryBytes}`,
        writeLimits.maxRegistryBytes
      );
    }
   ...
   * @param {{admitted: true, release: () => void}} [reservation] A slot already
   *   acquired through `reserveWrite`, which the request boundary holds while it
   *   reads the body. **Supplying one transfers ownership**: this function
   *   releases it exactly once, on every outcome, and the caller must not.
   *   Omitting it makes this function acquire and release its own, so a direct
   *   caller is bounded identically and a slot is never counted twice.
   * @returns {Promise<{studentId: string, activity: string, source: string}>}
   *   The created record, which is what the route serializes.
   *   Rejects with `code` `'INVALID_STUDENT_ID'`, `'STUDENT_NOT_FOUND'`,
   *   `'INVALID_ACTIVITY'`, `'ACTIVITY_ALREADY_RECORDED'`,
   *   `'ACTIVITY_PERSIST_FAILED'`, `'ACTIVITY_WRITE_QUEUE_FULL'`,
   *   `'STUDENT_ACTIVITY_LIMIT_REACHED'` or `'ACTIVITY_REGISTRY_FULL'` - the
   *   last three carrying the `limit` they reached as diagnostic detail. Never
   *   throws synchronously, so a caller can rely on a single rejection path.
   */
  const addActivity = (id, activity, reservation) => {
    // Ownership of a supplied reservation passes to this function here: from
    // this line on, every outcome below releases it exactly once, and the
    // caller must not.
    let slot = isWriteReservation(reservation) ? reservation : null;
    let studentId;
    let value;
    try {
      ...
      if (slot === null) {
        // No reservation: this caller reached the module directly rather than
        // through the request boundary, so the slot is acquired here and
        // exactly the same bounds apply to it.
        const acquired = reserveWrite(studentId);
        if (acquired.admitted !== true) throw errorForRefusal(acquired.refusal);
        slot = acquired;
      } else {
        // A reserved write already holds its queue slot, so only the quotas are
        // re-judged - they can have been reached while the body was read.
        const refusal = quotaRefusalFor(studentId);
        if (refusal !== null) throw errorForRefusal(refusal);
      }
    } catch (error) {
    ...
    const settled = enqueue(() => persistActivity(studentId, value));
    // The release rides on a **derived** promise with both handlers supplied,
    // so the caller still receives the original settlement and neither outcome
    // can surface as an unhandled rejection through the accounting.
    settled.then(
      () => { slot.release(); },
      () => { slot.release(); }
    );
 ...
 *   registry path. The four write bounds are forwarded to `fromData`, which
 *   applies `WRITE_LIMIT_DEFAULTS` to each one omitted. This module applies no
 *   other default and reads no environment variable.
...
const load = (options) => {
  ...
  return fromData({
    ...
    // Forwarded explicitly rather than by spreading `options`, so the write
    // bounds are visibly part of this module's contract with the composition
    // root. Each one omitted falls back to `WRITE_LIMIT_DEFAULTS` in `fromData`.
    maxPendingWrites: options.maxPendingWrites,
    maxActivitiesPerStudent: options.maxActivitiesPerStudent,
    maxRegistryRecords: options.maxRegistryRecords,
    maxRegistryBytes: options.maxRegistryBytes
  });
};

module.exports = { load, fromData, WRITE_LIMIT_DEFAULTS };
