...
/**
 * The name prefix of the scratch file the startup writability check creates and
 * immediately removes in the registry's own directory.
 *
 * Two properties of the name are deliberate, and both are load-bearing:
 *
 *   1. It is **not derived from the registry's basename**. A `<basename>.probe`
 *      name would fail with `ENAMETOOLONG` on a long-but-legal registry name
 *      whose own `<basename>.tmp` still fits under the platform's limit, which
 *      would refuse a start the write path would have handled perfectly well -
 *      a name-length technicality reported as an unwritable directory.
 *   2. It carries the process id and fresh randomness, so two startups probing
 *      one shared directory at the same instant cannot collide with each other,
 *      and a leftover from a process killed between the create and the unlink
 *      cannot make a later start fail either.
 */
const REGISTRY_PROBE_PREFIX = '.activities-registry-probe-';

/** Six bytes, so the hex suffix is twelve characters of collision resistance. */
const REGISTRY_PROBE_RANDOM_BYTES = 6;

/**
 * How many names the probe will try before giving up. `EEXIST` on a pid-and-
 * random name is already improbable; three attempts make a retry loop that
 * cannot spin, and exhausting them is reported as a probe that could not be
 * completed rather than as a directory proven unwritable.
 */
const REGISTRY_PROBE_ATTEMPTS = 3;

/**
 * The `libuv` codes that mean the filesystem **refused** the probe rather than
 * failed it: a mode denial, an ACL or ownership denial, and a read-only mount.
 * Each is a verdict on the directory, so each is fatal with the "not writable"
 * sentence; every other code is a probe that could not be completed.
 */
const REGISTRY_PROBE_DENIAL_CODES = ['EACCES', 'EPERM', 'EROFS'];
...
/**
 * A fresh, unique path for one probe attempt, inside the directory being
 * probed. See `REGISTRY_PROBE_PREFIX` for why the name is built this way rather
 * than from the registry's own basename.
 *
 * @param {string} directoryPath The registry's parent directory.
 * @returns {string} `<dir>/.activities-registry-probe-<pid>-<random hex>`.
 */
const registryProbePathFor = (directoryPath) =>
  path.join(
    directoryPath,
    `${REGISTRY_PROBE_PREFIX}${process.pid}-` +
      `${randomBytes(REGISTRY_PROBE_RANDOM_BYTES).toString('hex')}`
  );

/**
 * Removes the probe file this check created, best-effort.
 *
 * The create already proved what the check set out to prove, so a removal that
 * fails must not turn a usable configuration into a fatal startup error. It is
 * reported instead, on one escaped line naming the path, so an operator can
 * delete an empty file by hand - it holds no data, so nothing is lost by
 * leaving it either.
 *
 * `fs.unlinkSync` removes the directory entry itself and never follows it, and
 * the entry was created exclusively moments earlier, so this cannot reach
 * through a link (CWE-59). Deliberately not `removeOwnTemporaryFile`: that
 * function's message names the registry's `.tmp` scratch file, which an
 * operator greps for and which this is not.
 *
 * @param {string} probePath The probe file to remove.
 * @returns {void}
 */
const removeRegistryProbeFile = (probePath) => {
  try {
    fs.unlinkSync(probePath);
  } catch (cause) {
    // Already gone is success: nothing this check created needs removing twice.
    if (isErrorCode(cause, 'ENOENT')) return;
    reportArtifactProblem(
      `Could not remove the activity registry writability probe file ${probePath}: ` +
        `${describeCause(cause)}; it is empty and safe to delete by hand`
    );
  }
};

/**
 * The fatal refusal for a directory that answered the probe with a permission
 * denial. The sentence is unchanged from the one this check has always
 * produced, because it is what `README.md` documents and what an operator
 * greps for.
 *
 * @param {string} source The `Activity registry (<path>)` label.
 * @param {string} filePath The resolved registry path.
 * @param {string} directoryPath The registry's parent directory.
 * @param {unknown} cause The value the failing create raised.
 * @returns {Error} With `code === 'ACTIVITY_REPOSITORY_INVALID'`.
 */
const createRegistryNotWritableError = (source, filePath, directoryPath, cause) =>
...
/**
 * The fatal refusal for a probe that could not be completed at all - a
 * directory that vanished between the stat and the create, a path that stopped
 * being a directory, a full filesystem, or a name that stayed occupied for
 * every attempt. Equally fatal, because a write could not have succeeded
 * either, but worded as a probe that did not finish rather than as a directory
 * proven unwritable: the two say different things to whoever reads the line.
 *
 * @param {string} source The `Activity registry (<path>)` label.
 * @param {string} filePath The resolved registry path.
 * @param {string} directoryPath The registry's parent directory.
 ...
 * @returns {Error} With `code === 'ACTIVITY_REPOSITORY_INVALID'`.
 */
const createRegistryProbeFailedError = (source, filePath, directoryPath, cause) =>
  createRepositoryError(
    source,
    `the registry's directory ${directoryPath} could not be probed for ` +
      `writability, so an activity could never be persisted (${describeCause(cause)})`,
    { path: filePath },
    cause
  );
 ...
 *
 * **Writability is proven by writing, not by asking.** A uniquely named probe
 * file is created in the directory and removed again immediately, which is the
 * only question the filesystem answers the same way everywhere: `fs.accessSync`
 * with `W_OK` consults mode bits, so on Windows it reports a directory writable
 * whatever its ACLs say - a deny-write ACE was invisible to it, and such a
 * directory used to start a server that could never persist an activity. A
 * create either succeeds or is refused, on every platform, however that
 * platform spells its permissions.
 *
 * The probe is created exclusively - `'wx'` is `O_WRONLY | O_CREAT | O_EXCL`,
 * with the writer's own `TEMPORARY_FILE_MODE` - so it can never open, follow or
 * truncate an entry it did not create (the CWE-59 posture the write path keeps),
 * and it is never written to: the create is the whole proof. **Nothing of it
 * survives**; the descriptor is closed and the entry unlinked on every exit
 * path, and a removal that fails is reported on stderr rather than raised,
 * since writability is proven by then.
 *
 * The honest residual, unchanged: a directory writable at startup can still
 * refuse one particular write later - a mode changed under the running process,
 * a full disk, a lock. That case is what the write path's `ACTIVITY_PERSIST_FAILED`
 * rejection, and the `500` the routes module maps it to, exist for.
 *
 * @param {string} filePath The resolved registry path.
 * @returns {void}
 * @throws {Error} With `code === 'ACTIVITY_REPOSITORY_INVALID'` when the parent
 *   is missing, is not a directory, refuses the probe, or cannot be probed.
 */
const assertRegistryLocationUsable = (filePath) => {
  ...
  // The probe. `EEXIST` is the one failure that says nothing about the
  // directory - the name was taken, not the write refused - so it re-rolls the
  // name instead of deciding anything, bounded so the loop cannot spin.
  let lastCause;
  for (let attempt = 1; attempt <= REGISTRY_PROBE_ATTEMPTS; attempt += 1) {
    const probePath = registryProbePathFor(directoryPath);

    let descriptor;
    try {
      descriptor = fs.openSync(probePath, 'wx', TEMPORARY_FILE_MODE);
    } catch (cause) {
      lastCause = cause;
      if (isErrorCode(cause, 'EEXIST')) continue;
      if (REGISTRY_PROBE_DENIAL_CODES.some((code) => isErrorCode(cause, code))) {
        throw createRegistryNotWritableError(source, filePath, directoryPath, cause);
      }
      throw createRegistryProbeFailedError(source, filePath, directoryPath, cause);
    }

    // From here the probe file exists and belongs to this call. No byte is ever
    // written to it, so the only fault the release can meet is its own, and
    // neither the descriptor nor the directory entry may outlive this block.
    try {
      fs.closeSync(descriptor);
    } catch (cause) {
      // Not fatal, and not retried: the create already proved writability, and
      // closing a descriptor twice is undefined. The line exists so a leaked
      // handle is visible rather than silent.
      reportArtifactProblem(
        `Could not close the activity registry writability probe file ${probePath}: ` +
          `${describeCause(cause)}`
      );
    } finally {
      removeRegistryProbeFile(probePath);
    }
    return;
  }

  // Every attempt lost its name to something else. A write would have faced the
  // same contention, so this is fatal, with the last cause named.
  throw createRegistryProbeFailedError(source, filePath, directoryPath, lastCause);
};
