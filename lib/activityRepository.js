 ...
 * `[]`), which is the only file this feature writes and where an added record
 * is a one-line diff. A stored registry record is exactly
 ...
 * @returns {string} A short description, never empty.
 */
...
const createDuplicateError = (studentId, activity, origin) => {
  const error = new Error(
    `Student ${studentId} already holds activity "${activity}" (recorded by ${origin})`
  );
...
const createPersistError = (studentId, activity, targetPath, cause) => {
  const error = new Error(
    `Could not persist activity "${activity}" for ${KEY_HEADER_LABEL} ${studentId} ` +
 ...
 * cell absent from the worksheet XML, so `undefined` and `''` both mean "blank"
 * and are normalized to `''`. Any other type can only come from an injected
 * row, and it is refused rather than coerced: silently treating a numeric key as
 * blank would skip a row a maintainer believes is present.
 *
...
const readCell = (row, column, source, rowNumber) => {
  if (!isObject(row)) {
    throw createRepositoryError(
      source,
      `row ${rowNumber} must be an object keyed by column letter ` +
        `(received ${describeValue(row)})`,
      { row: rowNumber }
    );
  }
 ...
 * adding a record is a **one-line diff** in review. Only the two stored keys are
 * written, in a fixed order: `source` is derived and never stored.
 *
...
/**
 * Removes the scratch file after a failed write, on a best-effort basis: the
 * caller is already rejecting a request, and a cleanup failure must not replace
 * that diagnostic. No `.tmp` artifact may survive a failed write, which the
 * suite asserts both on the write-failure path and across the whole checkout.
 *
...
const discardTemporaryArtifact = (targetPath) => {
  ...
  try {
    ...
    // operator without turning cleanup into a second failure for the client.
 ...
 *   the header row at index 0, keyed by column letter. `registryRecords` is the
 *   registry in file order (absent or `null` means empty). `activitiesDataPath`
...
const fromData = (options) => {
  ...
  const registryInput = registryRecords === undefined || registryRecords === null
    ? []
    : registryRecords;
  if (!Array.isArray(registryInput)) {
    throw createRepositoryError(
      registryLabel,
      `registryRecords must be an array of records (received ${describeValue(registryRecords)})`
    );
  }
